...
const getImageUrl = (url) => {
  if (!url || typeof url !== 'string') return "/assets/images/placeholder-teal.png";
  const cleanUrl = url.trim();
  if (!cleanUrl) return "/assets/images/placeholder-teal.png";

  // 1. Protect bundled assets
  if (cleanUrl.startsWith('/assets/')) return cleanUrl;

  // 2. Extract filename (handles paths and absolute URLs)
  let filename = cleanUrl;
  if (cleanUrl.includes('/')) {
    filename = cleanUrl.split('/').pop();
  }

  // 3. If it's a full external URL (e.g. Unsplash), NOT from localhost or our known domains
  ...
  // 4. Return relative path for our uploads
...
const blogArticles = [
  {
    id: 1,
    title: "North Cyprus Property Market Trends for 2025",
    image: "https://images.unsplash.com/photo-1549517045-bc93de075e53?w=800&h=800&fit=crop",
    slug: "market-trends-2025"
  },
  {
    id: 2,
    title: "Luxury Living: Top Coastal Villas in Kyrenia",
    image: "https://images.unsplash.com/photo-1613490493576-7fde63acd811?w=800&h=800&fit=crop",
    slug: "luxury-villas-kyrenia"
  },
  {
    id: 3,
    title: "North Cyprus Citizenship & Real Estate Guide",
    image: "https://images.unsplash.com/photo-1541339907198-e08756ebafe3?w=800&h=800&fit=crop",
...
// ============================================
const Hero = ({ sliders, content, blocks }) => {
  ...
  const heroTitle = heroBlock?.title || getSetting('hero_title', 'CARIA ESTATES');
  const heroSubtitle = heroBlock?.subtitle || getSetting('hero_subtitle', 'Exclusive Mediterranean Living');
  const heroBtnText = getSetting('hero_btn_text', 'Explore Properties');
  const videoUrl = getImageUrl(heroBlock?.video_url || "/assets/videos/caria-vid.mp4");
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden flex items-center justify-center" data-testid="hero-section">
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-20 animate-bounce cursor-pointer">
        <ChevronDown size={30} className="text-white opacity-40" />
      </div>
    </section>
  );
};
...
// ============================================
const IntroSection = ({ content, blocks }) => {
  const introBlock = blocks?.find(b => b.block_type === 'intro');
  const introText = introBlock?.content || content?.find(c => c.content_key === 'intro_text')?.value_tr ||
  ...
  return (
    ...
    >
      <div className="relative h-64 overflow-hidden bg-caria-mint">
        <img
          src={getImageUrl(property.featured_image || property.image)}
          alt={property.title}
      ...
      <div className="p-5">
        ...
        <div className="flex items-center space-x-4 text-sm text-gray-500">
          <span className="flex items-center">
            ...
            {property.beds || property.beds_room_count || 0} bed
          </span>
          <span className="flex items-center">
            <Bath size={16} className="mr-1" />
            {property.baths || property.baths_count || 0} bath
          </span>
          <span className="flex items-center">
            <Square size={16} className="mr-1" />
            {property.area || property.closed_area || 0} m²
          </span>
        </div>
      </div>
    </Link>
...
// ============================================
const CuratedListings = ({ activeRegion, setActiveRegion, properties = [] }) => {
  ...
  const tags = ['All', ...new Set((properties || []).filter(p => p && p.tag).map(p => p.tag))];

  // Filter by region first
  const filteredByRegion = activeRegion === "DISCOVER ALL"
    ? properties
    : (properties || []).filter(p => p && p.region === activeRegion);

  // Then filter by tag
  const filteredProperties = activeTag === "All"
    ? filteredByRegion
    : (filteredByRegion || []).filter(p => p && p.tag === activeTag);
  ...
  return (
    <>
      <AdvancedSearchBar properties={properties} />
...
// ============================================
const ContactAgentSlider = ({ advisors = [] }) => {
  ...
  return (
    <section className="bg-caria-mint py-16 md:py-24" data-testid="agents-section">
      <div className="max-w-7xl mx-auto px-6">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {(filteredAgents || []).map((agent) => (
            ...
            >
              <img
                src={getImageUrl(agent.portraitUrl || agent.image_url || agent.thumbnail)}
...
const FeaturedPropertiesSlider = ({ featuredProperties }) => {
  ...
  const filteredProperties = activeFilter === 'All'
    ? (featuredProperties || [])
    : (featuredProperties || []).filter(p => p && (p.region === activeFilter || (p.location && p.location.includes(activeFilter))));
  ...
  return (
    <section className="bg-white py-16 md:py-24" data-testid="featured-section">
      <div className="max-w-[1440px] mx-auto px-8 lg:px-12">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
...
const Footer = ({ content }) => {
  ...
  return (
    <footer className="bg-[#0F5E63] text-white pt-24 pb-12 font-sans border-t border-white/5" data-testid="footer">
...
const CopyrightBar = () => {
  return (
    <div className="bg-[#0A4246] border-t border-white/5 py-8" data-testid="copyright">
      <div className="max-w-[1400px] mx-auto px-8 lg:px-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-[10px] tracking-[0.2em] uppercase text-white/30">
          <p>© 2026 Caria Estates İnternational.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16 text-center">
          {features.map((feature, idx) => (
            <div key={idx} className="flex flex-col items-center">
              ...
              <h3 className="font-serif text-lg tracking-[0.2em] text-caria-slate mb-4 uppercase">
                {feature.title}
              </h3>
...
const Home = () => {
      ...
      try {
        const fetchProperties = async () => {
          try {
            ...
            if (res.data && res.data.length > 0) {
              ...
              // Filter by new is_featured_slider flag, with legacy fallback to PREMIUM/EXCLUSIVE tags
              setFeatured(res.data.filter(p =>
                p.is_featured_slider === true ||
                p.is_featured_slider === 1 ||
                p.is_featured_slider === "Evet" ||
                (!p.hasOwnProperty('is_featured_slider') && (p.tag === 'PREMIUM' || p.tag === 'EXCLUSIVE'))
              ));
            }
          } catch (e) { console.error("Properties fetch failed", e); }
        };

        const fetchSliders = async () => {
          try {
            const res = await axios.get(`${API}/cms/sliders`);
            if (res.data) setSliders(res.data);
          } catch (e) { console.error("Sliders fetch failed", e); }
        };

        const fetchGuides = async () => {
          try {
            const res = await axios.get(`${API}/cms/country-guides`);
            if (res && res.data && Array.isArray(res.data)) setGuides(res.data);
          } catch (e) { console.error("Guides fetch failed", e); }
        };

        const fetchContent = async () => {
          try {
            const res = await axios.get(`${API}/cms/content`);
            if (res && res.data) setSiteContent(res.data);
          } catch (e) { console.error("Content fetch failed", e); }
        };

        const fetchHomepage = async () => {
          try {
            // BYPASS BLOCKS ROUTE: Fetch from pages logic
            const res = await axios.get(`${API}/cms/pages/home`);
            if (res.data && res.data.content_html) {
              try {
                const parsedBlocks = JSON.parse(res.data.content_html);
                setHomepageBlocks(parsedBlocks);
              } catch (e) {
                console.error("Home block parse failed", e);
              }
            }
          } catch (e) {
            console.error("Homepage page fetch failed", e);
          }
        };
        ...
        const fetchAdvisors = async () => {
          try {
            ...
            if (res && res.data && Array.isArray(res.data)) setAdvisors(res.data);
          } catch (e) {
            console.error("Advisors fetch failed", e);
          }
        };

        // Run all in parallel but independently
  ...
  if (!allProperties || !siteContent) {
    return (
      <div className="min-h-screen bg-aria-mint flex items-center justify-center">
        <div className="text-aria-slate tracking-[0.3em] uppercase animate-pulse">Loading experience...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      <Hero sliders={sliders} content={siteContent} blocks={homepageBlocks} />
      <IntroSection content={siteContent} blocks={homepageBlocks} />
      <StatsSection blocks={homepageBlocks} />
      <CuratedListings
        activeRegion={activeRegion}
        setActiveRegion={setActiveRegion}
        properties={allProperties}
      />
      <ContactAgentSlider advisors={advisors || []} />
      <FeaturedPropertiesSlider featuredProperties={featured || []} />
      <ServicesSection blocks={homepageBlocks || []} />
      <WhyChooseSection />
      <BlogSlider articles={blogArticles || []} />
      <JoinUsSection />
...
const PropertyDetailPage = () => {
  ...
  const images = (() => {
    ...
    const primaryImage = property.featured_image || property.image;
    return primaryImage
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative h-[70vh] md:h-[80vh] mt-16 md:mt-20 bg-caria-mint">
        <div className="relative w-full h-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 lg:p-10 bg-gradient-to-t from-black/70 via-black/40 to-transparent">
            <div className="max-w-7xl mx-auto">
              <p className="text-[#CCEBEA] text-xs md:text-sm tracking-[0.2em] uppercase font-medium mb-3 drop-shadow-lg">
                {property.location || property.region}
              </p>
              <h1 className="font-serif text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-white font-light mb-6 drop-shadow-2xl max-w-4xl" style={{ lineHeight: '1.1', letterSpacing: '-0.01em' }}>
                {property.title}
              </h1>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <p className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-light text-white tracking-tight drop-shadow-xl">
                  {formatPrice(property.price)}
                </p>
                <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-y-10 gap-x-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm md:text-base font-semibold text-caria-slate uppercase tracking-wider">
            ...
            {property.balcony && property.balcony !== "Yok" && (
              <div className="flex items-center gap-4">
                <LayoutGrid size={24} className="text-[#0F5E63] shrink-0" />
                <div className="flex flex-col">
            ...
            {property.distance_sea && (
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm md:text-base font-semibold text-caria-slate uppercase tracking-wider">{property.distance_sea} km</span>
                </div>
              </div>
            )}
            ...
            {property.distance_airport && (
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm md:text-base font-semibold text-caria-slate uppercase tracking-wider">{property.distance_airport} km</span>
                </div>
              </div>
            )}

            {/* Hospital */}
            {property.distance_hospital && (
              <div className="flex items-center gap-4">
                <ShieldCheck size={24} className="text-[#0F5E63] shrink-0" />
                <div className="flex flex-col">
                  <span className="text-[10px] text-gray-500 uppercase tracking-[0.2em] mb-1 font-bold">Hospital</span>
                  <span className="text-sm md:text-base font-semibold text-caria-slate uppercase tracking-wider">{property.distance_hospital} km</span>
                </div>
              </div>
            )}

            {/* School */}
            {property.distance_school && (
              <div className="flex items-center gap-4">
                <GraduationCap size={24} className="text-[#0F5E63] shrink-0" />
                <div className="flex flex-col">
                  <span className="text-[10px] text-gray-500 uppercase tracking-[0.2em] mb-1 font-bold">School</span>
                  <span className="text-sm md:text-base font-semibold text-caria-slate uppercase tracking-wider">{property.distance_school} km</span>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div className="text-gray-600 leading-relaxed space-y-4">
                  {property.full_address && (
                    <p className="font-semibold text-[#0F5E63] flex items-center gap-2 mb-2">
                      <MapPin size={16} /> {property.full_address}
                    </p>
                  )}
...
// ============================================
// SECTION: ADVISOR PROFILE (STRAND STYLE)
// ============================================
const AdvisorProfile = () => {
  ...
  useEffect(() => {
    const fetchAdvisor = async () => {
      try {
        const res = await axios.get(`${API}/advisors/${slug}`);
        setAdvisor(res.data);
      } catch (err) {
        console.error("Advisor not found:", slug);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-32 pb-20 md:pt-40 md:pb-28 bg-[#F4FAF9]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed max-w-3xl mx-auto">
            Professional property management services to ensure your investment is well-maintained and protected.
          </p>
        </div>
      </section>
      <section className="py-20 md:py-32">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="text-lg text-gray-600 mb-10">
            Our comprehensive management services include maintenance, rentals, and regular inspections.
            Contact us for a tailored management plan for your property.
          </p>
          <Link
            to="/contact"
            className="inline-block px-10 py-4 bg-[#1BAFA2] text-white text-sm tracking-wider uppercase rounded-md hover:bg-[#189890] transition-all shadow-sm"
...
// ============================================
// ONLINE VIEWING TOUR PAGE
// ============================================
const OnlineViewingTourPage = () => {
  return (
    <div className="min-h-screen bg-white">
      <div className="h-16 md:h-20" />
      <section className="pt-32 pb-20 md:pt-40 md:pb-28 bg-[#CCEBEA]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl text-caria-slate font-light mb-8">
            Online Viewing Tour
          </h1>
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed max-w-3xl mx-auto">
            Experience your future home from anywhere in the world with our personalized online viewing tours.
          </p>
        </div>
      </section>
      <section className="py-20 md:py-32">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="text-lg text-gray-600 mb-10">
            We offer live video tours via WhatsApp, Zoom, or FaceTime, allowing you to explore every detail of the property in real-time.
          </p>
          <Link
            to="/contact"
            className="inline-block px-10 py-4 bg-[#1BAFA2] text-white text-sm tracking-wider uppercase rounded-md hover:bg-[#189890] transition-all shadow-sm"
...
// ============================================
const ServicesSection = ({ blocks }) => {
  const defaultServices = [
    { title: "Property Search", desc: "Find your ideal home with our expert guidance.", icon: <Search size={24} /> },
    { title: "Legal Advice", desc: "Professional legal support for property transactions.", icon: <Check size={24} /> },
    { title: "Investment", desc: "Strategic advice for maximum ROI in Northern Cyprus.", icon: <Building2 size={24} /> }
  ];

  return (
    <section className="py-20 md:py-32 bg-caria-mint">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="font-serif text-3xl md:text-4xl text-caria-slate text-center mb-16">Our Services</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {((blocks && blocks.length > 0) ? (blocks || []) : (defaultServices || [])).map((service, idx) => (
            <div key={idx} className="p-8 border border-gray-100 rounded-lg hover:shadow-lg transition-all">
              <div className="text-[#1BAFA2] mb-6">{service.icon}</div>
              <h3 className="font-serif text-2xl text-caria-slate mb-4">{service.title || service.name}</h3>
              <p className="text-gray-600">{service.desc || service.description || service.content}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <BrowserRouter>
        ...
        <div className="mega-menu-blur">
          <Routes>
            ...
            {/* <Route path="/blog" element={<BlogPage />} /> */} {/* Assuming BlogPage is not defined or handled by DynamicPage */}
            {/* <Route path="/sell" element={<SellPropertyPage />} /> */} {/* Assuming SellPropertyPage is not defined or handled by DynamicPage */}
            <Route path="/viewing-trip" element={<CMSPage />} />
            <Route path="/viewing-trip/:slug" element={<CMSPage />} />
            <Route path="/page/:slug" element={<CMSPage />} />
            <Route path="/contact" element={<ContactPage />} />
            <Route path="/advisor/:slug" element={<AdvisorProfile />} />
            ...
            <Route path="/online-viewing-tour" element={<OnlineViewingTourPage />} />
            ...
            <Route path="/sell" element={<Home />} /> {/* Keeping original sell route for now */}
...
const CMSHeroBlock = ({ data }) => (
  <section className="relative h-[60vh] lg:h-[70vh] flex items-center justify-center overflow-hidden">
    <div className="absolute inset-0">
      <img 
        src={data.bgImage || "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=1920"} 
        alt="" 
        className="w-full h-full object-cover"
      />
    ...
    <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
      <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-white font-light tracking-tight mb-6 animate-in fade-in slide-in-from-bottom-4 duration-1000" style={{ lineHeight: '1.1' }}>
        {data.title}
      </h1>
      {data.subtitle && (
      ...
      {data.ctaText && (
        <Link 
          ...
          className="inline-flex items-center gap-2 bg-white text-caria-slate px-8 py-4 rounded-full font-medium hover:bg-caria-turquoise hover:text-white transition-all duration-300"
        >
          {data.ctaText}
          <ChevronRight size={20} />
        </Link>
...
const CMSTextSection = ({ data }) => (
  <section className="py-16 md:py-20 bg-white">
    <div className="max-w-4xl mx-auto px-6">
      {data.heading && (
        <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl text-caria-slate mb-8">
          {data.heading}
        </h2>
      )}
...
const CMSImageTextBlock = ({ data }) => (
  <section className="py-16 md:py-20 bg-[#F8F6F3]">
    <div className="max-w-6xl mx-auto px-6">
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${data.align === 'right' ? 'lg:flex-row-reverse' : ''}`}>
        <div className={data.align === 'right' ? 'lg:order-2' : ''}>
          <img 
            src={data.image} 
            alt={data.heading || ''} 
        ...
        <div className={data.align === 'right' ? 'lg:order-1' : ''}>
          {data.heading && (
            <h3 className="font-serif text-2xl md:text-3xl text-caria-slate mb-6">
              {data.heading}
            </h3>
          )}
          <div className="text-gray-600 leading-relaxed space-y-4">
...
const CMSQuoteBlock = ({ data }) => (
  <section className="py-16 md:py-24 bg-caria-slate">
    <div className="max-w-4xl mx-auto px-6 text-center">
      <div className="text-6xl text-caria-turquoise mb-6">"</div>
      <blockquote className="font-serif text-xl md:text-2xl lg:text-3xl text-white font-light italic leading-relaxed mb-8">
        {data.quote}
      </blockquote>
      {data.author && (
        <cite className="text-white/70 text-lg not-italic">— {data.author}</cite>
      )}
    </div>
  </section>
);

const CMSCTAStrip = ({ data }) => (
  <section className="py-16 md:py-20 bg-caria-turquoise">
    <div className="max-w-4xl mx-auto px-6 text-center">
      ...
      <Link 
        ...
        className="inline-flex items-center gap-2 bg-white text-caria-slate px-8 py-4 rounded-full font-medium hover:bg-caria-slate hover:text-white transition-all duration-300"
      >
...
const CMSContactForm = ({ data }) => {
  ...
  return (
    <section className="py-16 md:py-20 bg-white">
      <div className="max-w-2xl mx-auto px-6">
        {data.heading && (
          <h3 className="font-serif text-2xl md:text-3xl text-caria-slate mb-4 text-center">
            {data.heading}
          </h3>
        )}
        {data.description && (
          <p className="text-gray-600 text-center mb-10">{data.description}</p>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-caria-turquoise"
              />
              <input
                type="email"
                placeholder="Email Address"
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-caria-turquoise resize-none"
            />
            <button
              type="submit"
              className="w-full bg-caria-turquoise text-white py-4 rounded-lg font-medium hover:bg-caria-slate transition-colors"
            >
              Send Message
            </button>
          </form>
...
// Block Renderer
const CMSBlockRenderer = ({ block }) => {
  const data = typeof block.block_data === 'string' ? JSON.parse(block.block_data) : block.block_data;
  
  switch (block.block_type) {
    case 'HERO':
      return <CMSHeroBlock data={data} />;
    case 'TEXT_SECTION':
      return <CMSTextSection data={data} />;
    case 'IMAGE_TEXT':
      return <CMSImageTextBlock data={data} />;
    case 'QUOTE':
      return <CMSQuoteBlock data={data} />;
    case 'CTA_STRIP':
      return <CMSCTAStrip data={data} />;
    case 'CONTACT_FORM':
      return <CMSContactForm data={data} />;
    default:
      return null;
  }
};
...
const CMSPage = () => {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const res = await axios.get(`${API}/cms-blocks/page/${slug}`);
        setPage(res.data);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-caria-turquoise border-t-transparent rounded-full animate-spin mx-auto mb-4" />
  ...
  if (!page) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          <h1 className="font-serif text-4xl text-caria-slate mb-4">Page Not Found</h1>
          <Link to="/" className="text-caria-turquoise hover:underline">Return Home</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {page.blocks && page.blocks.map((block, idx) => (
        <CMSBlockRenderer key={block.id || idx} block={block} />
      ))}
          ...
          <div className="absolute inset-0">
            {page.banner_url.endsWith('.mp4') ? (
              <video autoPlay loop muted playsInline className="w-full h-full object-cover">
                <source src={getImageUrl(page.banner_url)} type="video/mp4" />
              </video>
            ) : (
              <img src={getImageUrl(page.banner_url)} alt={page.title} className="w-full h-full object-cover" />
            )}
            <div className="absolute inset-0 bg-black/40" />
          </div>
